...
interface ExportEventsCsvButtonProps {
  filterParams: Record<string, string>;
  className?: string;
}

export function ExportEventsCsvButton({ filterParams, className }: ExportEventsCsvButtonProps) {
  const [loading, setLoading] = useState(false);

  const onClick = useCallback(async () => {
    const params = new URLSearchParams(filterParams);
    const qs = params.toString();
    const url = qs ? `/api/events/export?${qs}` : '/api/events/export';
    setLoading(true);
    try {
      const res = await fetch(url, { credentials: 'include' });
      if (!res.ok) {
        toast.error('Could not export events. Try signing in again.');
        return;
      }
